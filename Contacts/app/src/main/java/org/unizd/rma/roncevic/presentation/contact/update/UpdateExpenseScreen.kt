...
import org.unizd.rma.roncevic.presentation.components.DropdownMenuUpdate
...
import org.unizd.rma.roncevic.presentation.components.showDatePickerUpdate
...
fun UpdateExpenseScreen(
    ...
    val cameraLauncher = rememberLauncherForActivityResult(
        ...
        updateExpenseViewModel.onImageUriChange(capturedImageUri.toString())
    }
    ...
    Scaffold (
        topBar = {
            TopAppBar(
                ...
                actions = {
                    Button(onClick = {
                        runBlocking {
                            updateExpenseViewModel.updateExpense(id)
        ...
        Column (
            ...
            TextInput(
                value = updateExpenseViewModel.name,
                onChange = {
                        value ->
                    updateExpenseViewModel.onNameChange(value)
            ...
            var text by remember { mutableStateOf(updateExpenseViewModel.amount) }
            ...
            TextInput(
                ...
                onChange = { value ->
                    if (value.isEmpty() || value.matches(doublePattern)) {
                        text = value
                        updateExpenseViewModel.onAmountChange(value)
                    }
                },
            ...
            DropdownMenuUpdate(updateExpenseViewModel = updateExpenseViewModel)

            Spacer(modifier = Modifier.width(16.dp))
            Spacer(modifier = Modifier.height(25.dp))

            showDatePickerUpdate(context = context, updateExpenseViewModel = updateExpenseViewModel)
